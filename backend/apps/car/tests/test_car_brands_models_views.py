...
class TestCarBrandModelViews(APITestCase):
    ...
    def setUp(self):
        # Створюємо користувачів з email, бо username не використовується
    ...
    def test_retrieve_brand(self):
        url = reverse("carbrand-detail", args=[self.brand.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
...
class TestCarModelViews(APITestCase):
    ...
    def test_create_model_allowed_for_staff(self):
        self.client.force_authenticate(user=self.staff_user)
        url = reverse("carmodel-list")
        data = {"brand": self.brand.id, "model": "Camry"}  # поле "brand" для ForeignKey
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_create_model_denied_for_normal_user(self):
        self.client.force_authenticate(user=self.normal_user)
        url = reverse("carmodel-list")
        data = {"brand": self.brand.id, "model": "Camry"}
        response = self.client.post(url, data)
        self.assertIn(response.status_code, [status.HTTP_403_FORBIDDEN, status.HTTP_401_UNAUTHORIZED])

    def test_update_model_allowed_for_staff(self):
        self.client.force_authenticate(user=self.staff_user)
        url = reverse("carmodel-detail", args=[self.model.id])
        data = {"brand": self.model.brand.id, "model": "UpdatedModel"}
        response = self.client.put(url, data)
        self.assertEqual(response.status_code, 200)
