...
const CreateCarPosterPage = () => {
  ...
  useEffect(() => {
    carService
      ...
      .catch(() => setBackendError({ detail: "Помилка при завантаженні брендів." }));
  }, []);
  ...
  const handleBrandChange = (brandId) => {
    ...
    if (numericId) {
      carService
        ...
        .catch(() => setBackendError({ detail: "Помилка при завантаженні моделей." }));
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!preparedData.brand_id) {
        setBackendError({ detail: "Не обрано бренд. Перевірте форму." });
        return;
      }

      if (!preparedData.model_id) {
        setBackendError({ detail: "Не обрано модель. Перевірте форму." });
        return;
      }

      const res = await carService.createCar(user.id, preparedData);

      // ✅ Показуємо повідомлення з бекенду
      const message = res.data?.message;
      const successText = message
        ? message.toLowerCase().includes("нецензур")
          ? `⚠ ${message}`
          : `✅ ${message}`
        : "✅ Оголошення створене успішно!";

      setSuccessMessage(successText);
      reset();

      // ✅ Запам’ятовуємо шлях для навігації (але не переходимо одразу)
      if (res.data?.id) {
        setRedirectPath(`/cars/${res.data.id}`);
      } else {
        setRedirectPath("/cars");
      }
    } catch (err) {
      if (err.response?.data) {
        setBackendError(err.response.data);
      } else if (err.message?.includes("<!DOCTYPE html>")) {
        setBackendError({ detail: "⚠ Внутрішня помилка сервера (500). Перевірте бекенд." });
      } else {
  ...
  // ✅ Користувач закриває повідомлення — тільки тоді переходимо
  ...
  return (
    <div className="flex justify-center items-center min-h-[80vh]">
      <div className="bg-gray-900 p-8 rounded-xl shadow-lg w-full max-w-lg">
        ...
        {successMessage && (
          <div
            className={`mb-4 p-3 rounded text-sm relative ${
          ...
          >
            ...
            <p className="text-xs mt-1 opacity-75">ℹ Закрийте повідомлення, щоб продовжити</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
          ...
          <label className="flex flex-col text-gray-100">
            ...
            {errors.brand && <span className="text-red-500 text-sm">Виберіть бренд</span>}
          </label>
          ...
          >
            {showNewBrand ? "✖ Приховати поле" : "👉 Не знайшли потрібний бренд? Натисніть тут"}
          </button>
          ...
          {selectedBrand && !showNewBrand && (
            <>
              <label className="flex flex-col text-gray-100">
                ...
                {errors.model && <span className="text-red-500 text-sm">Виберіть модель</span>}
              </label>
          ...
          <label className="flex flex-col text-gray-100">
            Валюта
            <select {...register("original_currency", { required: true })} className={inputClass}>
...
//   const handleBrandChange = (brandId) => {
//     const numericId = parseInt(brandId, 10);
//     console.log("📍 handleBrandChange brandId:", numericId);
//     setSelectedBrand(numericId);
//     resetField("model");
//     setModels([]);
//
...
//         .getModels({ brand: numericId })
//         .then((res) => {
//           console.log("📦 Models for brand", numericId, res.data);
//           setModels(res.data || []);
//         })
...
//       setSuccessMessage(null);
//
//       console.group("🚗 [CreateCarPosterPage Submit]");
//       console.log("📦 Raw form data:", data);
//
//       // 🔧 Заміна ключів на brand_id / model_id
//       const preparedData = {
//         brand_id: data.brand ? parseInt(data.brand, 10) : null,
//         model_id: data.model ? parseInt(data.model, 10) : null,
//         description: data.description || "",
...
//       const res = await carService.createCar(user.id, preparedData);
//
//       console.log("✅ Created car response:", res.data);
//
